<div class="col-sm-6 col-thumb {{ \Request::route()->getName() == 'frontend.courses' ? 'col-md-4' : 'col-md-3' }}">
  	<div class="thumbnail ">
    	...
    	<a class="position-relative btn-block course-img" href="{{route('frontend.course.show', $course)}}" data-toggle="popover" data-container="body" data-placement="right" id="{{$course->id}}">
			...
			<div class="profile-rating">
                ...
                @auth
                    @if(config('site_settings.site_enable_affiliate') && $course->price > 0)
                        <affiliate-link link="{{ route('frontend.course.show', ['course' => $course->slug, 'ref' =>auth()->user()->affiliate_id]) }}" inline-template>
                            <span class="pull-right">
                                <!--
                                <a href="#" data-toggle="tooltip" title="{{__('t.promote-this-course')}}">
                                    <i class="fa fa-money"></i>
                                </a>
                                -->
                                <span data-toggle="tooltip" title="{{__('t.promote-this-course')}}">
                                    <a href="#" data-toggle="modal" data-target="#affiliateLink-{{$course->id}}">
                                        <i class="fa fa-money"></i>
                                    </a>
                                </span>
                                
                                @include('courses.partials._affiliate_modal')
                            </span>
                        </affiliate-link>
                    @endif
                @endauth
