...
@section ('title', __('t.blog-categories'))
...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <strong>{{__('t.blog-categories')}}</strong>
            <small class="text-muted">{{__('t.manage-blog-categories')}}</small>
            
            <div class="btn-toolbar float-right" role="toolbar" data-toggle="tooltip" title="{{ __('t.add-new') }}">
                <a href="#" class="btn btn-sm btn-success ml-1" data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#createCategoryModal" >
                    ...
                    @foreach($catArray as $v)
                        <li class="dd-item dd3-item" data-id="{{ $v->id }}">
                            ...
                            <div class="dd3-content">
                                {{ $v->name }} <span class="badge badge-secondary">{{ $v->posts->count() }}</span>
                                <span class="pull-right">
                                     @if($v->slug == 'site-pages')
                                         <i class="icon-lock text-secondary"></i>
                                     @endif
                                    <a href="{{ route('admin.blog.categories.edit', $v) }}" class="edit">
                                        <i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" title="{{__('t.edit') }}"></i>
                                    </a>
                                    
                                    @if(!$v->posts->count() && $v->slug != 'site-pages')
                                        <a href="{{ route('admin.blog.destroy', $v) }}"
                                             data-method="delete"
...
@section('modals')
    ...
    <div class="modal fade" id="createCategoryModal" tabindex="-1" role="dialog" aria-labelledby="CreateCategory" aria-hidden="true">
        <div class="modal-dialog modal-success" role="document">
            {!! Form::open(['route' => ['admin.blog.categories.store'], 'method'=>'POST', 'class' => 'form-horizontal create-category-form']) !!}
